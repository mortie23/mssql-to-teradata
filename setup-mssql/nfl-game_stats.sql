...
-- Usage:   Invoke-Sqlcmd -InputFile "nfl-game_stats.sql" -ServerInstance "localhost\SQLEXPRESS" | Out-File -FilePath "nfl-game_stats.txt"
--          Invoke-Sqlcmd -InputFile "nfl-game_stats.sql" -ServerInstance "localhost\SQLEXPRESS" -Username mortimer -Password mortimer | Out-File -FilePath "nfl-game_stats.txt" 
...
BULK INSERT [nfl].[GAME_STATS]
FROM N'$(rootPath)\data\game_stats.csv'
WITH
(
  FIRSTROW = 2,
    FIELDTERMINATOR = ',', 
...
CREATE TABLE [nfl_si].[GAME_STATS] (
  ...
  , CONSTRAINT FK_GAME_STATS_TEAM_ID FOREIGN KEY ([TEAM_ID])
    REFERENCES [nfl_si].[TEAM_LOOKUP] ([TEAM_ID])
) ON [PRIMARY]
GO
-- Transform from landing into source image with correct data types
INSERT INTO [nfl_si].[GAME_STATS]
